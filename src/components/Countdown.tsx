...
const Countdown = ({ targetDate }: CountdownProps) => {
  ...
  const WrapperComponent = (props: any) => (
    <div className="flex flex-col gap-10 text-lg font-mono bg-slate-500 p-4 rounded-2xl text-white mx-10 w-full ">
      {props.children}
    </div>
  );
  ...
  return (
    <WrapperComponent>
      <Text text="Menanti Hari" font="main" size="3xl" weight="bold" color="text-white" />
      <div className="flex flex-wrap gap-4 w-full justify-around">
        {Object.entries(timeLeft).map(([unit, value]) => (
          <div key={unit} className="flex flex-col items-center justify-center">
            <Text text={value.toString()} font="main" size="5xl" weight="bold" color="text-white" />
            <Text text={unit} font="main" size="2xl" weight="medium" color="text-white" />
          </div>
        ))}
      </div>
    </WrapperComponent>
